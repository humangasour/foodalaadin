...
function azh_print_switch_mode_button($post) {
    $settings = get_option('azh-settings');
    if (is_object($post) && in_array($post->post_type, array_keys($settings['post-types']))) {
...
function azh_get_object() {
    ...
    if (empty($azh)) {
        ...
        $azh = array(
            ...
            'device_prefix' => 'lg',
            'responsive' => true,
            'editor_toolbar' => array('boldButton', 'italicButton', 'linkButton', 'leftButton', 'centerButton', 'rightButton', 'orderedListButton', 'unorderedListButton', 'sizeSelector', 'colorInput'),
            'elements_hierarchy' => true,
            'table_editor' => true,
            'recognition' => false,
            'i18n' => array(
                'shortcodes_and_elements' => esc_html__('Shortcodes and Elements', 'azh'),
...
function azh_editor_scripts() {
    ...
    wp_enqueue_script('azexo_html', plugins_url('js/azexo_html.js', __FILE__), array('underscore', 'azh_htmlparser', 'jquery-ui-sortable', 'jquery-ui-autocomplete'), false, true);
    ...
    wp_enqueue_script('ace', plugins_url('js/ace/ace.js', __FILE__), false, true);
...
function azh_add_meta_boxes($post_type, $post) {
    ...
    if (!isset($_GET['azh'])) {
        ...
        if (is_array($dirs)) {
            foreach ($dirs as $dir => $uri) {
                if (is_dir($dir) && in_array($post_type, apply_filters('azh_meta_box_post_types', array_keys($settings['post-types'])))) {
    ...
    if (in_array($post_type, apply_filters('azh_meta_box_post_types', array_keys($settings['post-types'])))) {
        if (isset($_GET['azh']) && $_GET['azh'] == 'customize') {
            wp_enqueue_script('azh_admin_frontend', plugins_url('js/admin-frontend.js', __FILE__), array('jquery', 'underscore', 'azh_htmlparser', 'jquery-ui-mouse', 'jquery-ui-sortable', 'jquery-ui-autocomplete'), false, true);
            $azh = azh_get_object();
        ...
        <div class="azh-sections">
            ...
            foreach ($sections as $path => $name) {
                ...
                if (file_exists($preview)) {
                    ...
                    ?><div class="azh-section no-image <?php print isset($general_sections[$path]) ? 'general' : ''; ?>" data-url="<?php print esc_attr($url); ?>" data-order="<?php print isset($files_order[$path]) ? $files_order[$path] : 999999  ?>" data-path="<?php print esc_attr(ltrim(str_replace($dir, '', $path), '/')) ?>" data-dir="<?php print esc_attr($dir) ?>" data-dir-uri="<?php print esc_attr($sections_uri[$path]) ?>"><div><?php print esc_html($name) ?></div></div><?php
                        }
                    }
                    ?>        
        </div>
        ...
        <div class="azh-elements" style="display: none;">  
            ...
            foreach ($elements as $path => $name) {
                ...
                if (file_exists($preview)) {
                    ...
                    ?><div class="azh-element no-image  <?php print isset($general_elements[$path]) ? 'general' : ''; ?>" data-url="<?php print esc_attr($url); ?>" data-order="<?php print isset($files_order[$path]) ? $files_order[$path] : 999999  ?>" data-path="<?php print esc_attr(ltrim(str_replace($dir, '', $path), '/')) ?>" data-dir="<?php print esc_attr($dir) ?>" data-dir-uri="<?php print esc_attr($elements_uri[$path]) ?>"><div><?php print esc_html($name) ?></div></div><?php
                        }
                    }
                    ?>        
        </div>
...
function azh_admin_body_class($classes) {
    global $pagenow;
    $settings = get_option('azh-settings');
    if (in_array($pagenow, array('post.php', 'post-new.php')) && in_array(get_post_type(), array_keys($settings['post-types']))) {
        $post = get_post();
        ...
        if (isset($_GET['azh']) && $_GET['azh'] == 'customize') {
            ...
            <div id="azexo-html-library">
                <div class="azh-library-actions">
                    ...
                    <?php if ($azh['elements_hierarchy']): ?>
                        <div class="azh-hierarchy" title="<?php esc_html_e('Elements hierarchy', 'azh'); ?>"></div>
                    <?php endif; ?>
                    ...
                    <a href="<?php print get_dashboard_url(); ?>" class="azh-dashboard" title="<?php esc_html_e('WordPress dashboard', 'azh'); ?>"></a>
                </div>
                <div class="azh-panel azh-general">
                </div>
...
function azh_content_begin($content) {
    ...
    if (is_object($post)) {
        if ($post->post_type == 'azh_widget') {
            ...
            $settings = get_option('azh-settings');
            if (in_array($post->post_type, array_keys($settings['post-types'])) && get_post_meta($post->ID, 'azh', true)) {
                $content = azh_get_post_content($post);
                remove_filter('the_content', 'wpautop');
            }
        }
        ...
        if ($post->post_type == 'azh_widget') {
            ...
            if (in_array($post->post_type, array_keys($settings['post-types'])) && get_post_meta($post->ID, 'azh', true)) {
                ...
                if (!empty($content) && false === strpos($content, ' data-section=')) {
                    $content = '<div data-section="' . uniqid() . '">' . $content . '</div>';
                }
                add_filter('the_content', 'wpautop');
            }
        }
...
function azh_init() {
    if (is_admin()) {
        ...
        if (is_array($settings['post-types'])) {
...
function azh_the_content($content) {
    ...
    $settings = get_option('azh-settings');
    if (get_post_type() == 'azh_widget' || (in_array(get_post_type(), array_keys($settings['post-types'])) && get_post_meta(get_the_ID(), 'azh', true))) {
...
function azh_builder_scripts() {
    ...
    wp_enqueue_script('azh-frontend-customization-options', plugins_url('frontend-customization-options.js', __FILE__), array(), false, true);
    wp_enqueue_script('simplemodal', plugins_url('js/jquery.simplemodal.js', __FILE__), array('jquery'), false, true);
    ...
    wp_register_script('iris', '/wp-admin/js/iris.min.js', array('jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch'), '1.0.7', 1);
    wp_register_script('wp-color-picker', "/wp-admin/js/color-picker$suffix.js", array('iris'), false, 1);
    wp_enqueue_style('wp-color-picker', "/wp-admin/css/color-picker$suffix.css");
    wp_enqueue_script('wp-color-picker-alpha', plugins_url('js/wp-color-picker-alpha.js', __FILE__), array('wp-color-picker'), false, true);
            ...
            if ($name == 'AZH_Widget') {
                ...
                foreach ($instances as $number => $instance) {
                    if (isset($instance['post']) && is_numeric($instance['post'])) {
                        ...
                        if ($widget_post) {
                            $azh_widgets_edit['#' . $widget_obj->id_base . '-' . $number] = add_query_arg(array('azh' => 'customize', 'azhf' => $post->ID), get_edit_post_link($widget_post));
                        }
                    }
                }
            }
...
function azh_scripts() {
    ...
    wp_enqueue_script('countdown', plugins_url('js/jquery.countdown.js', __FILE__), array('jquery'), false, true);
    wp_enqueue_script('maskedinput', plugins_url('js/jquery.maskedinput.js', __FILE__), array('jquery'), false, true);
    wp_enqueue_style('swiper', plugins_url('css/swiper.css', __FILE__));
    wp_enqueue_script('swiper', plugins_url('js/swiper.js', __FILE__), array(), false, true);
    wp_enqueue_style('animate', plugins_url('css/animate.css', __FILE__));
    wp_enqueue_style('magnific-popup', plugins_url('css/magnific-popup.css', __FILE__), array(), null);
    wp_enqueue_script('magnific-popup', plugins_url('js/jquery.magnific-popup.js', __FILE__), array('jquery'), false, true);
    wp_enqueue_script('azh-scrollReveal', plugins_url('js/scrollReveal.js', __FILE__), array('jquery'), false, true);


    $user = wp_get_current_user();
    if (in_array('administrator', (array) $user->roles) || ($post && isset($post->post_author) && $post->post_author == get_current_user_id())) {
        wp_enqueue_script('azh_admin_frontend', plugins_url('js/admin-frontend.js', __FILE__), array('jquery', 'underscore', 'jquery-ui-mouse', 'jquery-ui-sortable', 'jquery-ui-resizable', 'jquery-ui-draggable', 'jquery-ui-autocomplete'), false, true);
        wp_enqueue_script('azh_frontend', plugins_url('js/frontend.js', __FILE__), array('jquery', 'imagesloaded', 'azh_admin_frontend'), false, true);
        ...
        wp_enqueue_script('azh_frontend', plugins_url('js/frontend.js', __FILE__), array('jquery', 'imagesloaded'), false, true);
        wp_enqueue_style('azh_frontend', plugins_url('css/frontend.css', __FILE__));
    ...
    $fonts_url = azh_get_google_fonts_url(azh_get_post_settings(), azh_get_widgets_content() . azh_get_post_content($post));
    if ($fonts_url) {
...
//        $user = wp_get_current_user();
//        if (in_array('administrator', (array) $user->roles)) {
//            update_option('azh-shortcodes', stripslashes_deep($_POST['shortcodes']));
//        }
//    }
//    wp_die();
//}
...
function azh_admin_bar_menu($wp_admin_bar) {
    if (!(isset($_GET['azh']) && $_GET['azh'] == 'customize')) {
        ...
        $settings = get_option('azh-settings');
        if ((in_array(get_post_type(), array_keys($settings['post-types'])) && get_post_meta(get_the_ID(), 'azh', true)) || (is_singular() && get_post_type() == 'azh_widget')) {
            $wp_admin_bar->add_node(array(
...
add_filter('post_class', 'azh_post_class', 11);
...
function azh_get_post_settings($post = false) {
    ...
    if ($azh_content) {
        ...
        if ($post) {
            return azh_get_content_settings(azh_get_widgets_content() . azh_get_post_content($post));
        } else {
            return false;
        }
    }
...
function azh_get_content_scripts($content) {
    ...
    $post_scripts = apply_filters('azh_get_content_scripts', $post_scripts);
    return $post_scripts;
}
...
function azh_get_post_scripts() {
    ...
    if ($azh_content) {
        ...
        $post = get_post();
        if ($post) {
            return azh_get_content_scripts(azh_get_widgets_content() . azh_get_post_content($post));
        } else {
...
function azh_get_google_fonts_url($azh_settings = false, $content = false) {
...
function azh_process_form() {
    ...
    if (wp_verify_nonce($nonce[0], 'azh-form-nonce-' . $nonce[1])) {
        ...
        if (isset($settings['form-submission-notification']) && $settings['form-submission-notification'] == 'immediately') {
    ...
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
        ...
        switch ($response['status']) {
            case 'redirect' :
                ...
                break;
            case 'success' :
                if (isset($_POST['success-redirect'])) {
                    exit(wp_redirect($_POST['success-redirect']));
                }
                break;
            case 'error' :
                if (isset($_POST['error-redirect'])) {
        ...
        if (is_array($library['elements'])) {
            foreach ($library['elements'] as $element_file => $name) {
                if (strlen($element_file) - strlen($file) == strrpos($element_file, $file)) {
                    ...
                    $content .= '<div data-element="' . esc_attr(ltrim(str_replace($library['elements_dir'][$element_file], '', $element_file), '/')) . '" style="margin: auto; max-width: ' . $element_width . '">';
                    $content .= azh_get_file($element_file, $library['elements_uri'][$element_file]);
                    $content .= '</div>';
                    break;
                }
...
function azh_add_post() {
    if (isset($_POST['post_title']) && isset($_POST['post_type'])) {
        ...
        if ($user_id) {
            ...
            if (isset($_POST['general_element'])) {
                $general_element = sanitize_text_field($_POST['general_element']);

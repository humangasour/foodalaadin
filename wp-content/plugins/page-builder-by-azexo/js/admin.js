                    ...
                    if (azh.edit_post_frontend_link) {
                        ...
                        $window.on('load', function() {
                                    ...
                                    azh.set_device_width = function(prefix) {
                                        if (azh.post_frontend_frame) {
                                            ...
                                            if (prefix === 'lg') {
                                                ...
                                                azh.post_frontend_frame.animate({
                                                    left: left + 'px',
                                                    width: azh.device_prefixes[prefix].width + 1
                                                });
                                                $controls_container.animate({
                                                    left: left + 'px',
                                                    width: azh.device_prefixes[prefix].width
                                                });
                                            }
                                        }
                                        $('.azh-controls-container .azh-responsive [data-prefix="' + prefix + '"]').trigger('click');
                                        $('.azh-context-menu .azh-responsive [data-prefix="' + prefix + '"]').trigger('click');
                                        $('.azh-context-menu').on('azh-showed', function(){
                            ...
                            azh.post_frontend_frame.on('load', function() {
                                ...
                                var $controls_container = $('.azh-controls-container').css({
                                    top: '0',
                                    left: '0',
                                    width: '100%',
                                    position: 'absolute'
                                ...
                                $dialog_document.on('heartbeat-send', function(event, data) {
...
//            }
        ...
        azh.open_elements_dialog = function(callback) {
            ...
            if (dialog_init()) {
                $$.simplemodal.close();
                var $modal = $$('<div class="azh-dialog"></div>');
                $$('<div class="azh-dialog-title">' + azh.i18n.shortcodes_and_elements + '</div>').appendTo($modal);

                var form = $$('<div class="azh-elements-form"></div>').appendTo($modal);
                var $categories_select = null;
                var tags_select = null;
                var categories = {};
                categories[azh.i18n.general] = [];
                ...
                for (var tag in azh.shortcodes) {
                    if (!('show_in_dialog' in azh.shortcodes[tag] && azh.shortcodes[tag]['show_in_dialog'] === false)) {
                        if ('category' in azh.shortcodes[tag]) {
                            ...
                            categories[azh.i18n.general].push(azh.shortcodes[tag]);
                        }
                    }
                }
//                if (categories[azh.i18n.general].length == 0) {
//                    delete categories[azh.i18n.general];
//                }
                if ($$('.azh-library .azh-elements .azh-element').length) {
                    categories[azh.i18n.elements] = [];
                }
                var tabs = $$('<div class="azh-tabs"></div>').appendTo(form);
                var tabs_buttons = $$('<div></div>').appendTo(tabs);
                var tabs_content = $$('<div></div>').appendTo(tabs);
                ...
                for (var category in categories) {
                    var id = makeid();
                    ids[category] = id;
                    $$('<span><a href="#' + id + '">' + category + '</a></span>').appendTo(tabs_buttons);
                }
                var child_suggestions = [];
                for (var category in categories) {
                    var tab = $$('<div id = "' + ids[category] + '"></div>').appendTo(tabs_content);
                    if (category == azh.i18n.general) {
                        general_tab = tab;
                    }
...
//                                callback('shortcode', '[' + $$(this).data('settings')['base'] + '] [/' + $$(this).data('settings')['base'] + ']');
//                            } else {
                            callback('shortcode', '[' + $$(this).data('settings')['base'] + ']');
//                            }
                            $$.simplemodal.close();
                        });
                    }
                    if (category === azh.i18n.elements) {
                        ...
                        $$('.azh-library .azh-elements .azh-element').each(function() {
                            ...
                            button.appendTo(tab).on('click', function() {
                                ...
                                $$.get($$(button).data('url'), function(data) {
                                    ...
                                    callback($$(button).data('path'), data, !$$(button).is('.general'));
                                    $$.simplemodal.close();
                                });
                            });
                ...
                if (general_tab) {
                    var general_elements = general_tab.children();
                    general_elements.detach();
                    general_elements.sort(function(element1, element2) {
                        var order1 = parseInt($$(element1).attr('data-order'), 10);
                        var order2 = parseInt($$(element2).attr('data-order'), 10);
                        return (order1 < order2) ? -1 : (order1 > order2) ? 1 : 0;
                    });
                    general_tab.append(general_elements);
                }
                tabs_init($$(tabs));
                ...
                $modal.simplemodal({
                    ...
                    closeClass: "azh-close",
                    onShow: function() {
                        if (azh.default_category && $categories_select.find('[value="' + azh.default_category + '"]').length) {
